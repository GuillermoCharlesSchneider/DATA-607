...
MERGED$EVTMembers[is.na(MERGED$EVTMembers)] <- 0
MERGED$FTMembers[is.na(MERGED$FTMembers)] <- 0
...
#MERGED <- MERGED
MERGED$ComplaintRate <- MERGED$TotalComplaints / MERGED$TotalMembers * 100
...
```{r}
GroupedStewards <- MERGED %>% group_by(Account.Id, FT,PT,EVT, Total, Number.of.Complaints) %>% summarise(totalLeaderPoints = sum(Leader.Points) ,.groups = 'drop')
GroupedStewards
```
...
```{r}
ggplot(MERGED, aes(x=FT, y=Number.of.Complaints)) + 
    geom_point(alpha=0.7)+
  geom_smooth(method=lm , color="lightblue", fill="#69b3a2", se=TRUE) 
```

```{r}
CL1 <- ggplot(MERGED, aes(x=Leader.Points, y=Number.of.Complaints, size = Total)) + 
    geom_point(alpha=0.7)

CL2 <- ggplot(GroupedStewards, aes(x=totalLeaderPoints, y=totalComplaints, size = Total)) + 
    geom_point(alpha=0.7)
