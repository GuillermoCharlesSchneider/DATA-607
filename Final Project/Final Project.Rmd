...
TotalComplaints <- TotalComplaints[ -c(3:5) ]
...
# NA's to 0
MERGED$EVT[is.na(MERGED$EVT)] <- 0
MERGED$FT[is.na(MERGED$FT)] <- 0
MERGED$PT[is.na(MERGED$PT)] <- 0
...
MERGED <- MERGED[which(MERGED$Total != 0),]

#keep just the year-long stewards
MERGED <- MERGED[which(MERGED$numofDaysSteward == 365),]

#remove the EVT workers
MERGED <- MERGED[which(MERGED$EVT == 0),]
...
```{r}
GroupedStewards <- MERGED %>% group_by(Account.Id, Total) %>% summarise(totalComplaints = Number.of.Complaints,totalLeaderPoints = sum(Leader.Points) ,.groups = 'drop')
GroupedStewards
```
...
```{r}
ggplot(MERGED, aes(x=Total, y=Number.of.Complaints)) + 
...
m <- train(Number.of.Complaints ~ Turnout.Points + Activity.Points + FT + PT + Total,
           data = MERGED,
           trControl = tc,
           method = "rpart",
           tuneGrid = expand.grid(cp = 0))

m2 <- train(Number.of.Complaints ~ FT + PT + Total,
           data = MERGED,
           trControl = tc,
           method = "rpart",
           tuneGrid = expand.grid(cp = 0))
